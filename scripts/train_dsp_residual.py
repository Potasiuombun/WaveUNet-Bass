#!/usr/bin/env python3
"""Train D2 branch: fast DSP baseline + tiny bounded residual model."""
...
def normalize_config(config: Dict[str, Any]) -> Dict[str, Any]:
    cfg = dict(config)
    cfg.setdefault("experiment_name", "dsp_residual")
    ...
    cfg["dsp"].setdefault("target_level_db", -14.0)
    cfg["dsp"].setdefault("max_gain_db", 12.0)
    cfg["dsp"].setdefault("min_gain_db", -12.0)
    cfg["dsp"].setdefault("peak_limit", 0.99)
    cfg["dsp"].setdefault("smoothing", {"enabled": True, "alpha": 0.9})
    cfg["dsp"].setdefault("compression", {"enabled": False, "threshold_db": -6.0, "ratio": 2.0})

    cfg["model"].setdefault("hidden_channels", 16)
    cfg["model"].setdefault("num_layers", 4)
    cfg["model"].setdefault("kernel_size", 7)
    cfg["model"].setdefault("activation", "leaky_relu")
    cfg["model"].setdefault("max_residual", 0.2)
    ...
    cfg["training"].setdefault("output_root", "outputs/dsp_residual")
    return cfg


def loss_batch(outputs: torch.Tensor, targets: torch.Tensor, l1_loss: L1Loss, nmse_loss: NMSELoss, cfg: Dict[str, Any]) -> torch.Tensor:
    return float(cfg["loss"]["l1_weight"]) * l1_loss(outputs, targets) + float(cfg["loss"]["nmse_weight"]) * nmse_loss(outputs, targets)


def main() -> None:
    parser = argparse.ArgumentParser(description="Train DSP+tiny residual branch")
    parser.add_argument("--config", default="configs/dsp_residual.yaml")
    ...
    for epoch in range(1, int(cfg["training"]["num_epochs"]) + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, start=1):
            ...
            with torch.no_grad():
                dsp_out, _ = dsp.process_batch(inp)

            residual = model(dsp_out)
            out = apply_residual_with_peak_safety(dsp_out, residual, peak_limit=float(cfg["dsp"]["peak_limit"]))
            loss = loss_batch(out, tgt, l1_loss, nmse_loss, cfg)
            ...
            metrics = evaluate_batch(out.detach(), tgt.detach())
            train_loss += float(loss.item())
            train_nmse += float(metrics["batch_nmse"])
            train_mae += float(metrics["batch_mae"])
            n_train += 1
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader, start=1):
                ...
                out = apply_residual_with_peak_safety(dsp_out, residual, peak_limit=float(cfg["dsp"]["peak_limit"]))
                loss = loss_batch(out, tgt, l1_loss, nmse_loss, cfg)
                metrics = evaluate_batch(out, tgt)
                val_loss += float(loss.item())
                val_nmse += float(metrics["batch_nmse"])
                val_mae += float(metrics["batch_mae"])
                n_val += 1
    ...
    with open(Path(cfg["training"]["output_root"]) / "LATEST_DSP_RESIDUAL.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, sort_keys=True)
