#!/usr/bin/env python3
"""Train DD1 branch: DDSP-style interpretable controller."""
...
from src.dsp.bandsplit import FixedBandSplitter
...
def normalize_config(config: Dict[str, Any]) -> Dict[str, Any]:
    cfg = dict(config)
    cfg.setdefault("experiment_name", "ddsp_controller")
    ...
    cfg["model"].setdefault("max_tilt_db", 3.0)
    cfg["model"].setdefault("envelope_enabled", True)
    cfg["model"].setdefault("min_envelope", 0.7)
    cfg["model"].setdefault("max_envelope", 1.3)
    ...
    cfg["training"].setdefault("output_root", "outputs/ddsp_controller")
    return cfg
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train DDSP-style controller")
    parser.add_argument("--config", default="configs/ddsp_controller.yaml")
    ...
    for epoch in range(1, int(cfg["training"]["num_epochs"]) + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, start=1):
            ...
            controls = model(inp)
            out = apply_ddsp_controls(
                x=inp,
                splitter=splitter,
                controls=controls,
                peak_limit=float(cfg["dsp"]["peak_limit"]),
            )
    ...
    with open(Path(cfg["training"]["output_root"]) / "LATEST_DDSP_CONTROLLER.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, sort_keys=True)
